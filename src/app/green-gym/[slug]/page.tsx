...
import { ArrowLeft, Check, Phone, MessageSquare, Send, MapPin, Wrench } from "lucide-react";
...
export default function GreenGymProductDetailPage({
  params,
}: {
  params: { slug: string };
}) {
  ...
  if (!product || product.category !== "green-gym") {
    notFound();
  }

  const [activeImage, setActiveImage] = useState(product.image);
  const relatedProducts = PRODUCTS.filter(
    (p) => p.category === "green-gym" && p.id !== product.id
  ).slice(0, 3);

  const whatsappMsg = `Hello AMEY INDUSTRIES, I am interested in ${product.name}. Please share details and quotation.`;
    ...
    <div className="py-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-12 text-left">
      ...
      <div>
        ...
        >
          ...
          <span>Back to Green Gym Catalog</span>
        </Link>
      </div>
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          {product.features && (
            <div className="space-y-2">
              <h3 className="text-xs font-bold text-industrial-textMuted uppercase tracking-wider">Key Features</h3>
          ...
          {product.installationDetails && (
            <div className="p-4 rounded-xl bg-industrial-lightGreenBg border border-emerald-200 text-xs text-industrial-text space-y-1">
              <h4 className="font-bold text-industrial-green flex items-center">
                <Wrench className="w-4 h-4 mr-1.5" />
                Installation Specifications
              </h4>
              <p>{product.installationDetails}</p>
            </div>
          )}
      ...
      {relatedProducts.length > 0 && (
        <div className="space-y-6 pt-8 border-t border-industrial-border">
          <h3 className="text-2xl font-bold text-industrial-text">Other Green Gym Equipment</h3>
